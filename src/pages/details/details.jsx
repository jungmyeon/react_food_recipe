...
export default function Details(){
    ...
    useEffect(()=>{
        ...
        async function getFoodDetail(){
            ...
            const data = await res.json();          // json 문자열로 줄테니가 자바스크립트에 맞게 변환하자

            console.log(data)       // 잘들어왔으면 foodDetailData에 담자 (setFoodDetailData를 통해)
            if(data?.data){             // 잘 들어왔니?
    ...
    }, [])      // useEffect가 처음 켜졌을때만 동작하게끔 [] 를 넣어준다 ([] 안넣으면 바뀔때마다 update 발동)
    // useEffect : mount, update, unmount ==> update에 대해서 동작으로 안하고싶다 하면 []
            ...
            <div className='content-container'>
                ...
                <div>
                    <button onClick={()=>{hAddToFavorite(foodDetailData?.recipe)}} className='favorites-btn'>
                        {/* 해당 아이디가 favoritesList에 없으면 '즐겨찾기 추가' 
                            있으면 '즐겨찾기에서 제거' */
                            favoritesList && favoritesList.length > 0 && favoritesList.findIndex(item=>item.id === 
                                foodDetailData.recipe?.id) === -1 ? '즐겨찾기에 추가' : '즐겨찾기에 제거'
                            }
                    </button>
                </div>
                <div>
                    ...
                    <ul className='recipe-content'>
                            {
                                // map을 통해서 들어있는 만큼만 반복하며 li 태그 생성
