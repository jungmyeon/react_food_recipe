...
export default function GlobalState({children}){
    ...
    async function hSubmit(event){
        ...
        {
            const res = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes?search=${searchParam}`);    // get 요청 (REST API GET요청)
            const information = await res.json();      // REST API는 JSON문자열로 전달하니까 사용할 수 있는 자료형으로 변경
    ...
    {
        let copyFavoritesList = [...favoritesList]      // 배열 통쨰로 분해했다가 다시 배열로 만들어서 대입
        ...
        if(index === -1){
            ...
            copyFavoritesList.splice(index);        // 즐겨찾기 리스트에 있으면 제거
        }
    ...
    return(
        <GlobalContext.Provider value={{searchParam, setSearchParam, hSubmit, foodList,setFoodList, 
        foodDetailData, setFoodDetailData, favoritesList, hAddToFavorite}}>
            {children}
